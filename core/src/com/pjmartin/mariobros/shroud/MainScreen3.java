...
public class MainScreen3 implements Screen, ContactListener {
    ...
    public MainScreen3(MarioBros play) {
        ...
        hud = new HUD(play.a, MarioBros.lives, MarioBros.marioScore,"1-3" );

        mapLoad = new TmxMapLoader();
        map = mapLoad.load("level3.tmx");
    ...
    public void render(float delta) {
        ...
        if(player.dead) {
            MarioBros.marioScore = hud.score;
            play.setScreen(new MainScreen3(play));
            dispose();
        }
    ...
    public void beginContact(Contact contact) {
        ...
        if(firstbody != null && secondbody !=null){
            ...
            if(firstbody.getUserData() == "Mario's head" && secondbody.getUserData() == "Question"){
                //(rectangle.getX() + rectangle.getWidth()/2) / MarioBros.Pixels, (rectangle.getY() +  2 * rectangle.getHeight()) / MarioBros.Pixels))
